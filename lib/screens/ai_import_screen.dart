...
class _AiImportScreenState extends State<AiImportScreen> {
  ...
  final String aiPrompt =
      """Rol: Eres un asistente experto en GTA Online y extractor de datos estructurados.
Objetivo: Analizar imágenes de garajes del jugador y generar ÚNICAMENTE un JSON válido basado en los catálogos oficiales proporcionados.
Fuentes de Verdad (Uso Obligatorio):
- Vehículos: https://raw.githubusercontent.com/DevS4nsan/GTAOnlineTracker/refs/heads/main/assets/data/vehicles.json
- Propiedades: https://raw.githubusercontent.com/DevS4nsan/GTAOnlineTracker/refs/heads/main/assets/data/properties.json

Reglas Estrictas de Extracción:
- Nombre de Propiedad (garage_name): Usa EXACTAMENTE el valor de la clave name del archivo properties.json. NUNCA agregues sufijos, prefijos o niveles al nombre (ej. omite "Garage 1", "Sótano 2"). Si no es identificable, usa "Desconocido".
- Nombre de Vehículo (car_name): Debe coincidir EXACTAMENTE con el nombre del catálogo oficial (vehicles.json).
- Manejo de Slots: Lista los vehículos visualmente en orden. Ignora los espacios que digan "Vacío" o "Disponible", pero respeta el hueco numérico en el conteo del Slot.
- Desplazamiento Matemático (Offsets): Si el usuario especifica que la captura es de un garaje secundario, AJUSTA el Slot inicial según las matemáticas del juego y la instrucción del usuario (Ej. Oficinas: Garaje 1 = 1-20; Garaje 2 = 21-40; Garaje 3 = 41-60).
- Formato de Salida: Devuelve EXCLUSIVAMENTE el JSON. DEBES envolver tu respuesta en un bloque de código Markdown válido usando json al inicio y  al final. No incluyas saludos ni explicaciones, solo el bloque de código.
...
Estructura JSON Requerida:
{
  ...
  Future<void> _loadPropertiesCatalog() async {
    ...
    final List<String> upgradableTypes = [
      'Executive Offices',
      'Retro Arcades',
      'Casino Penthouse',
    ];
    ...
    setState(() {
      _catalogProperties = data
          .where(
            (e) =>
                (e['capacity'] ?? 0) > 0 || upgradableTypes.contains(e['type']),
          )
          .map(
            (e) => {
              "id": e['id'],
